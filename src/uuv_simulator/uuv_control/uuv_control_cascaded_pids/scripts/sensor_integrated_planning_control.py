...
class IntegratedPlanningControl:
    def __init__(self):
        ...
        # Get the goal position from the parameter server, default to the origin
        self.goal = np.array(rospy.get_param('~goal_position', [0.0, 0.0, -15.0]))
        ...
        self.marker_pub = rospy.Publisher('/goal_orientation_vector', Marker, queue_size=10)
    ...
    def odometry_callback(self, msg):
        ...
        # Update R vector to point towards the goal instead of the origin
        self.R = np.array([msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z]) - self.goal
        ...
        goal_marker.pose.position.z = self.goal[2]
        goal_marker.pose.orientation.x = 0.0
        goal_marker.pose.orientation.y = 0.0
        goal_marker.pose.orientation.z = 0.0
        goal_marker.pose.orientation.w = 1.0
        goal_marker.scale.x = 0.5  # Size of the sphere
        goal_marker.scale.y = 0.5
        goal_marker.scale.z = 0.5
        goal_marker.color.a = 2.0  # Don't forget to set the alpha!
        goal_marker.color.r = 0.0
        goal_marker.color.g = 1.0
        goal_marker.color.b = 0.0
        self.marker_pub.publish(goal_marker)

        # Publish markers for both the goal and current vectors
        self.publish_vector_marker(self.R, 'R_vector', [1, 0, 0])  # Red for R vector
        self.publish_vector_marker(self.r_i, 'r_i_vector', [0, 1, 0])  # Green for r_i vector
