...
class DPControllerLocalPlanner(object):
    ...
    * `go_to_incremental` (*type:* `uuv_control_msgs.GoToIncremental`)
    ...
    def get_idle_circle_path(self, n_points=30, radius=30):
        ...
        * `radius` (*type:* `float`): Circle radius in meters
        ...
        if self._idle_circle_center is None:
            ...
            self._idle_circle_center = (pose.pos) + radius * frame[:, 1].flatten()
    ...
    def get_idle_helical_path(self, n_points=150, radius=30, z_step=0.1, N=3):  
        ...
        for idx, i in enumerate(np.linspace(u_init, u_init + N, n_points)):  
            wp = uuv_waypoints.Waypoint(
            x=self._idle_circle_center[0] + radius * np.cos(phi(i)),
            y=self._idle_circle_center[1] + radius * np.sin(phi(i)),
            z=self._idle_z + idx * z_step,  
            max_forward_speed=0.8 * self._max_forward_speed,
            inertial_frame_id=self.inertial_frame_id)
            wp_set.add_waypoint(wp)
        return wp_set
    ...
    def get_idle_lissajou_path(self, n_points=50, a=3, b=3, delta=np.pi/2, freq_x=3, freq_y=5, z_amplitude=10):
        pose = deepcopy(self._vehicle_pose)
    ...
    # Generate Lissajous curve
        ...
        y = b * np.sin(freq_y * t)
        z = z_amplitude * np.sin(t) + pose.pos[2]
    ...
    def get_idle_fo8_path(self, n_points=100, amplitude=30):
        ...
        if self._idle_circle_center is None:
            ...
            [0, 0, 1]])
            # self._idle_circle_center = (pose.pos + 0.8 * self._max_forward_speed * frame[:, 0].flatten()) + radius * frame[:, 1].flatten()
            self._idle_circle_center = (pose.pos) * frame[:, 1].flatten()
    ...
    # Generate figure-of-eight curve
        x = amplitude * np.sin(t) + pose.pos[0]
        y = amplitude * np.sin(2*t) + pose.pos[1]
        z = np.full_like(x, pose.pos[2])  # Keep the z-coordinate constant at the vehicle's current depth
        ...
        * `t` (*type:* `float`): Timestamp
        ...
        if not self._station_keeping_on and self._traj_running:
            ...
            if not self._thrusters_only and not self._is_teleop_active and rospy.get_time() - self._start_count_idle > self._timeout_idle_mode:
                ...
                wp_set = self.get_idle_circle_path() 
